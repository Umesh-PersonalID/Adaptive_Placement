...
ALPHA_CRIT = 0.5  # reduce repulsion to critical neighbors
V = 0.1
MASS = 1.0
DELTA_T = 0.05
MAX_STEP = 0.05
...
class HybridDeployment:
    ...
    def compute_forces(self, neighbors):
        ...
        for i in range(NUM_ROBOTS):
            ...
            # Sort neighbors by distance and define critical neighbors
            dists = [(j, float(self.distance(self.positions[i], self.positions[j]))) for j in nbrs]
            sorted_nbrs = sorted(dists, key=lambda x: x[1])
            critical = [j for j, _ in sorted_nbrs[:min(K_NEIGHBORS, len(nbrs))]]
            
            # Apply forces to all neighbors
            for j, d_ij in dists:
                if d_ij == 0:
                    continue
                    
                # Unit vector from i to j: u_ij = (pos[j] - pos[i]) / d_ij
                u_ij = (self.positions[j] - self.positions[i]) / max(d_ij, EPSILON)
                
                # 1) Coverage repulsion (all neighbors)
                if j in critical:
                    # Reduced repulsion for critical neighbors
                    f_cov = -(ALPHA_CRIT * K_COVER) * u_ij / (d_ij**2 + EPSILON)
                else:
                    # Full repulsion for non-critical neighbors
                    f_cov = -(K_COVER) * u_ij / (d_ij**2 + EPSILON)
                
                forces[i] += f_cov
                
                # 2) Degree-maintenance attraction (critical neighbors only)
                if j in critical and d_ij >= Q * COMM_RANGE:
                    # Rescue attraction when critical neighbor is near comm limit
                    f_deg = (K_DEGREE) * u_ij / (d_ij**2 + EPSILON)
                    forces[i] += f_deg
    ...
    def update_visualization(self):
        ...
        if final_coverage >= 99.9:
            ...
            ax.imshow(vis_grid, cmap=self.cmap, norm=self.norm, origin='lower')
