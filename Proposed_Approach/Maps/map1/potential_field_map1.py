...
# Load positions from position.txt
positions = []
...
def compute_obstacle_repulsion(pos, grid_data, repulsion_strength=1, repulsion_radius=10):
    forces = np.zeros_like(pos)
    shape = grid_data.shape

    for i, (x, y) in enumerate(pos.astype(int)):
        xmin, xmax = max(0, x - repulsion_radius), min(shape[0], x + repulsion_radius + 1)
        ymin, ymax = max(0, y - repulsion_radius), min(shape[1], y + repulsion_radius + 1)

        for nx in range(xmin, xmax):
            for ny in range(ymin, ymax):
                if grid_data[nx][ny] != 1:
                    continue
                vec = pos[i] - np.array([nx, ny])
                dist = np.linalg.norm(vec)
                if dist < 1e-2:
                    continue
                force_mag = repulsion_strength / (dist ** 2 + 1e-5)
...
def compute_forces(pos, neighbors):
    ...
    obstacle_repulsion = compute_obstacle_repulsion(pos, grid)
    forces += obstacle_repulsion
        ...
        for i in range(self.num_robots):
            if is_in_obstacle(new_positions[i], self.grid):
                ...
                if normal is not None:
                    ...
                    # Apply clipped movement again
                    step = self.velocities[i] * DELTA_T
                    norm = np.linalg.norm(step)
                    if norm > max_step:
                        step = step * (max_step / norm)
                    new_positions[i] = self.positions[i] + step
