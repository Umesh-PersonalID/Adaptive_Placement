...
class IncrementalDeployment:
    ...
    def policy4_selection(self):
        """Select deployment location using Policy 4:
        Push into unknown space from frontier, maximizing coverage"""
        ...
        for (x, y), (dx, dy) in frontier_directions:
            
            tx, ty = x + dx, y + dy
            
            if (0 <= tx < self.grid_size[0] and 0 <= ty < self.grid_size[1] and
                self.occupancy_grid[tx, ty] == -1 and self.occupancy_grid[tx, ty] != 1 ):
...
def visualize(deployment):
    ...
    def update(frame):
        ...
        for robot in deployment.robots:
            if robot.deployed:
                circle = plt.Circle((robot.position[1], robot.position[0]), 20, 
...
deployment = IncrementalDeployment(grid_size=(250, 250), num_robots=48, sensor_range=20)
