...
class IncrementalDeployment:
    def __init__(self, grid_size=(90, 70), num_robots=30, sensor_range=5):
        ...
        if self.robots:
            self.robots[0].position = (int(0.73*self.sensor_range), int(0.73*self.sensor_range))
            self.robots[0].deployed = True
            self.update_grids()
    
    def initialize_robots(self):
        for i in range(self.num_robots):
            self.robots.append(Robot(i, (int(0.73*self.sensor_range), int(0.73*self.sensor_range)), self.sensor_range))
    ...
    def next_center(self):
        ...
        cols =  int(self.grid_size[0] // (1.73 * radius))  + 1
 
        dx = np.sqrt(3)  * radius
        dy = 1.5 * radius
        for row in range(rows):
            for col in range(cols):
                y = int(0.73*radius) + int(col * dx + (radius if row % 2 else 0))
                x = int(0.73*radius) + int(row * dy)
