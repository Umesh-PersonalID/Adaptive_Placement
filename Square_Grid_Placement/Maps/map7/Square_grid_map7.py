...
import math
from Grid_generator_map7 import grid
import time
import sys
overlap = 0
count = 0
...
class IncrementalDeployment:
    def __init__(self, grid_size=(90, 70), num_robots=30, sensor_range=5):
        ...
        if self.robots:
            self.robots[0].position = (int(self.sensor_range), 36)
            self.robots[0].deployed = True
            self.update_grids()
    
    def initialize_robots(self):
        for i in range(self.num_robots):
            self.robots.append(Robot(i, (int(self.sensor_range), 36), self.sensor_range))
...
end_time = time.time()
deployment = IncrementalDeployment(grid_size=(300, 300), num_robots=112, sensor_range=15)
