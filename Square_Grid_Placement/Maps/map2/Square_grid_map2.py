...
import math
from Grid_generator_map2 import grid
import time
...
class IncrementalDeployment:
    def __init__(self, grid_size=(90, 70), num_robots=30, sensor_range=5):
        ...
        if self.robots:
            self.robots[0].position = (int(self.sensor_range), int(self.sensor_range))
            self.robots[0].deployed = True
            self.update_grids()
    
    def initialize_robots(self):
        for i in range(self.num_robots):
            self.robots.append(Robot(i, (int(self.sensor_range), int(self.sensor_range)), self.sensor_range))
    ...
    def next_center(self):
        ...
        rows = int(((self.grid_size[1] - 2*radius))//(np.sqrt(2)*radius)) + 2
        cols =  int(((self.grid_size[0] - 2*radius))//(np.sqrt(2)*radius)) + 2
        num_robots = rows*cols
        dx = int(np.sqrt(2)*radius)
        dy = int(np.sqrt(2)*radius)
...
def visualize(deployment):
    ...
    def update(frame):
        ...
        img = np.zeros((deployment.grid_size[1], deployment.grid_size[0], 3))
...
end_time = time.time()
deployment = IncrementalDeployment(grid_size=(300,157), num_robots=90, sensor_range=15)
...
print("Saved deployed robot positions to position.txt")