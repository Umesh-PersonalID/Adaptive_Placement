...
class GreedyDeployment:
    ...
    def init_visualization(self):
        self.im = self.ax1.imshow(self.occupancy_grid.T, cmap=self.cmap, norm=self.norm, 
    ...
    def update_visualization(self):
        ...
        if deployed_positions:
            ...
            if self.robot_scatter:
                self.robot_scatter.set_offsets(np.c_[x, y])
            else:
                self.robot_scatter = self.ax1.scatter(x, y, c='green', s=10, label='Robots')
    ...
    def create_animation(self):
        ani = animation.ArtistAnimation(self.fig, self.animation_frames, interval=300, 
                                       blit=True, repeat_delay=1000)
        return ani


# Run the deployment
deployment = GreedyDeployment(grid_size=(300, 300), num_robots=150, sensor_range=15)
