...
from grid_generator_map10 import grid
import time
...
class GreedyDeployment:
    def __init__(self, grid_size=(300, 300), num_robots=150, sensor_range=15):
        ...
        self.fig.suptitle('Greedy Robot Deployment Algorithm - Map 14')
    ...
    def save_positions(self, filename):
        """Save robot positions to txt file in the format: x, y"""
...
deployment = GreedyDeployment(grid_size=(285, 300), num_robots=156, sensor_range=15)
...
deployment.fig.text(0.02, 0.90, f'Overlap: {overlap_pct:.2f}%', fontsize=14, fontweight='bold',
...
# Save the figure
deployment.fig.savefig('Coverage_map14.png', dpi=300, bbox_inches='tight')

# Save positions to txt file
deployment.save_positions('position_map14.txt')
